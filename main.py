class FiveLetters():
    ...
    def input_coincedences(self):
        print('Input coincidences which mean\n"0"-non in this word,"-1"-wrong position,"1"-correct position')
    ...
    def elaboration_list_with_word(self):
        list_temp = []
        list_temp1 = [self.new_word[count] for count, val in enumerate(self.truth) if val==1] #list with correct letters
        ...
        if len(list_temp) == 0:
            print('Smthng goes wrong, idk. Probably your dict isnt full') # вообще это местро про мой баг или про то, что слова нет в списке
    ...
    def main(self):
        ...
        if len(self.list_with_words) == 1: #probably this if does not work, need to test, but too lazy)))
